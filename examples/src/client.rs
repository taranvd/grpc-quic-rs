...
fn load_client_tls() -> TlsConfig {
    let cert_der = std::fs::read("cert.der").expect("failed to read cert.der; start the server first!");
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let requests = vec![
        ...
        HelloRequest { name: "David".to_string() },
    ];
    ...
    let mut response_stream = client.lots_of_replies(HelloRequest {
        name: "Eve".to_string(),
    }).await?.into_inner();
    ...
    let bidi_requests = vec![
        HelloRequest { name: "Frank".to_string() },
        HelloRequest { name: "Grace".to_string() },
        HelloRequest { name: "Heidi".to_string() },
    ];
